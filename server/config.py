...
# ── 로컬 모델 ─────────────────────────────────────────────────────────────────────
LOCAL_BASE_URL: str = os.getenv("LOCAL_BASE_URL") or _get("local.base_url", "http://localhost:11434/v1")

# ── 실행 엔진 ─────────────────────────────────────────────────────────────────────
MAX_RETRIES:     int        = int(_get("execution.max_retries", 2))
NODE_TIMEOUT:    int        = int(_get("execution.node_timeout", 120))
PASS_THRESHOLD:  float      = float(_get("execution.pass_threshold", 0.85))
AUTO_THRESHOLD:  float      = float(_get("execution.auto_threshold", 0.6))
ESCALATE_TAGS:   set[str]   = set(
    _get("execution.escalate_tags", ["high-stakes", "medical", "legal"])
)

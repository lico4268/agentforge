...
5. **`in:`의 이름은 `nodes:` 전체 어딘가의 `out:`이 만들면 된다.** flow 순서상
   먼저 나오는 노드가 나중 노드의 출력을 입력으로 받아도 오류가 아니다 — 루프를
   통해 피드백이 돌아오는 정상적인 경우이기 때문이다(§5.1). 검증은 그래프 전체를
   보고 하므로, 어떤 노드의 `out:`에도 없는 이름을 `in:`에 쓴 경우만(오타) 파서
   오류가 된다. 런타임에서 아직 값이 없으면 `None`으로 들어오고, `llm_step.py:71`이
   이미 `None` 값을 프롬프트 블록에서 건너뛴다.
...
회차의 판단이 입력으로 들어와야 한다. `reasoning`이 `feedback`을 입력으로
받되, 그걸 만드는 `review`는 flow 순서상 `reasoning`보다 뒤에 있다 — 검증은
그래프 전체를 보고 통과시킨다(§3.1 규칙 5):

```yaml
nodes:
  reasoning:
    in:  [task, plan, feedback]      # 1회차엔 아직 값이 없어 None
    out: [answer]
    prompt: |
      계획을 따라 풀어라.
...
- `in:`에 적힌 이름을 `nodes:` 전체 어디의 `out:`에서도 찾을 수 없음(flow 순서상
  뒤에 나오는 노드의 `out:`도 인정한다 — 루프 피드백이 정상 케이스이므로).
  메시지: `node {name!r} reads {want!r} but no node writes it — add it to
  some node's out:`
